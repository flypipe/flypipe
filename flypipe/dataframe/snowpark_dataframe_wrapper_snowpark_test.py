...
class TestSnowparkDataFrameWrapperSnowpark:
    ...
    def test_get_column_flypipe_type(self, snowflake_session):
        ...
        df = snowflake_session.create_dataframe([], schema=schema)
        ...
        assert isinstance(
            df_wrapper.get_column_flypipe_type("C2"), Long
        )  # ByteType converted to Long
        assert isinstance(df_wrapper.get_column_flypipe_type("C3"), Binary)
        assert isinstance(
            df_wrapper.get_column_flypipe_type("C4"), Long
        )  # IntegerType converted to Long
        assert isinstance(
            df_wrapper.get_column_flypipe_type("C5"), Long
        )  # ShortType converted to Long
        assert isinstance(df_wrapper.get_column_flypipe_type("C6"), Long)
        assert isinstance(
            df_wrapper.get_column_flypipe_type("C7"), Double
        )  # FloatType converted to Double
    ...
    def test_cast_column(self, snowflake_session):
        ...
        df = snowflake_session.create_dataframe(pd.DataFrame({"col1": [1, 0, None]}))
        df_wrapper = SnowparkDataFrameWrapper.get_instance(snowflake_session, df)
        ...
        # Compare as pandas DataFrames
        assert_frame_equal(
            df_wrapper.df.to_pandas(), expected_df.to_pandas(), check_dtype=False
        )
    ...
    def test_cast_column_decimal(self, snowflake_session):
        ...
        df = snowflake_session.create_dataframe(
            pd.DataFrame({"col1": [5678.12345, 999.11111, 1.2345678, None]})
        )
        df_wrapper = SnowparkDataFrameWrapper.get_instance(snowflake_session, df)
        ...
        schema_dict = {
            field.name: field.datatype for field in df_wrapper.df.schema.fields
        }
    ...
    def test_cast_column_date_with_snowflake_format(self, snowflake_session):
        ...
        df = snowflake_session.create_dataframe(
            pd.DataFrame({"col1": ["2024-01-15", "2024-12-31", "2025-06-30"]})
        )
        df_wrapper = SnowparkDataFrameWrapper.get_instance(snowflake_session, df)
    ...
    def test_cast_column_datetime_with_snowflake_format(self, snowflake_session):
        ...
        datetime_type = DateTime(
            format="YYYY-MM-DD HH24:MI:SS", format_mode=DateFormat.SNOWFLAKE
        )
