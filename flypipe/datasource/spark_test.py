...
class TestSparkDataSource:
    ...
    def test_load(self, spark):
        ...
        df_expected = spark.createDataFrame(
            schema=StructType([StructField('c1', DecimalType(16, 2))]), data=[(PythonDecimal(1),)])
    ...
    def test_multiple_sources(self, spark):
        ...
        df_expected = spark.createDataFrame(schema=StructType([
        ...
        ]), data=[(PythonDecimal(0), PythonDecimal(1), PythonDecimal(2), PythonDecimal(3))])
    ...
    def test_datasource_consolidate_columns(self, spark, mocker):
        ...
        @node(
            type="pyspark",
            dependencies=[
                spark_dummy_table.select('c0')
            ],
            output=Schema([
                Column('c0', Decimal(10, 2), 'dummy')
            ])
        )
        def t1(dummy_table1):
            return dummy_table1
