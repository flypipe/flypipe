...
data = [
    ("id1", datetime.strptime("2022-01-01", "%Y-%m-%d")),
...
schema = StructType(
    [StructField("id", StringType(), True), StructField("date", DateType(), True)]
)
...
class TestSparkDataSource:
    ...
    def test_load_filter_end_date(self):
        df = Spark(spark).load(
            "spark_datasource", time_travel_column="date", end_time_travel="2022-01-02"
        )
