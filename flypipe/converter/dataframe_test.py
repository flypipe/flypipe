...
class TestDataFrameConverter:
    ...
    def test_convert_pandas_to_pandas_on_spark(self, spark, pandas_df, pandas_on_spark_df):
        df_ = DataFrameConverter(spark).convert(pandas_df, DataFrameType.PANDAS_ON_SPARK)
    ...
    def test_convert_pandas_on_spark_to_pandas(self, spark, pandas_on_spark_df, pandas_df):
        df_ = DataFrameConverter(spark).convert(pandas_on_spark_df, DataFrameType.PANDAS)
        assert_dataframes_equals(df_, pandas_df)

    def test_convert_pandas_on_spark_to_pyspark(self, pandas_on_spark_df, pyspark_df):
        df_ = DataFrameConverter(spark).convert(pandas_on_spark_df, DataFrameType.PYSPARK)
        assert_dataframes_equals(df_, pyspark_df)
    ...
    def test_convert_pyspark_to_pandas_on_spark(self, pyspark_df, pandas_on_spark_df):
        df_ = DataFrameConverter(spark).convert(pyspark_df, DataFrameType.PANDAS_ON_SPARK)
        assert_dataframes_equals(df_, pandas_on_spark_df)
