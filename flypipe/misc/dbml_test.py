...
def assert_strings_equal_ignore_whitespace(str1, str2):
    assert str1.replace("\n", "").replace("\t", "").replace(" ", "") == str2.replace("\n", "").replace("\t", "").replace(" ", ""), \
        f"Strings are not equal (ignoring spaces, \\n and \\t)."
...
class TestDBML:
    ...
    def test_nodes_upstream_are_mapped(self):
        ...
        @node(
            ...
            output=Schema(
                ...
                .one_to_one(A.output.node_a_col1)
            )
        )
        def B(**dfs):
            pass
        ...
        @node(
            type="pandas",
            output=Schema(
                Column("node_c_col1", String(), "description node_c_col1")
                .one_to_one(B.output.node_b_col1)
            )
        )
        def C():
            pass
    ...
    def test_nodes_independents_are_mapped(self):
        ...
        @node(
            type="pandas",
            output=Schema(
                Column("node_d_col1", String(), "description node_d_col1")
    ...
    def test_only_nodes_in_list_and_with_cache_and_tags_are_mapped(self):
        ...
        @node(
            ...
            output=Schema(
                Column("node_e_col1", String(), "description node_e_col1")
            )
        )
        def E():
            pass
        ...
        dbml = build_dbml([B], only_nodes_with_cache=True, only_nodes_with_tags=["dbml"])
        expected_dbml = """
        Table B {
