...
from graphql import NonNullTypeNode

from flypipe.node import Node
from flypipe.run_context import RunContext
from flypipe.schema.column import Relationship, RelationshipType
...
class Table:
    ...
    def table_description(self):
        return "" if not self.node.description else f"\n\tNote: '''{self.node.description}'''"

    @property
    def table_name(self):
        return self.node.function.__name__ if self.node.cache is None else self.node.cache.name
            ...
            if (
                ...
                or
                    (
                        ...
                        and
                        (not only_nodes_with_cache or (only_nodes_with_cache and node_.cache is not None))
                    )
            ):
    ...
    while frontier:
        ...
        for column in node_.output.columns:
            for fk_node, fk in column.foreign_keys.items():
                if (
                    is_tag_in_considered(fk_node.parent.tags, only_nodes_with_tags)
                    and
                    (not only_nodes_with_cache or (only_nodes_with_cache and fk_node.parent.cache is not None))
                ):
