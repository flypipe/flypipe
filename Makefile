...
FLYPIPE_TEST_SPARK_CONNECTION=SPARK
...
coverage:
	docker-compose -f $(LOCAL_DIR)/docker-compose.yaml run --remove-orphans --entrypoint "" flypipe-jupyter sh -c "export FLYPIPE_TEST_SPARK_CONNECTION=$(FLYPIPE_TEST_SPARK_CONNECTION) && pytest --rootdir flypipe -n $(PYTEST_THREADS) -k '_test.py' --cov=flypipe --no-cov-on-fail --cov-fail-under=$(MIN_COVERAGE) flypipe"
.PHONY: coverage

test:
	docker-compose -f $(LOCAL_DIR)/docker-compose.yaml run --remove-orphans --entrypoint "" flypipe-jupyter sh -c "export FLYPIPE_TEST_SPARK_CONNECTION=$(FLYPIPE_TEST_SPARK_CONNECTION) && pytest -n $(PYTEST_THREADS) -k '_test.py' -vv $(f) --rootdir flypipe"
.PHONY: test
