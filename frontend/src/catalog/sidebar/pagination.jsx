...
const Pagination = ({maxPage, pageGroupSize, handleClickPage}) => {
    ...
    useEffect(() => {setPageGroup(initialPageGroup)}, [initialPageGroup]);

    const onPageClick = useCallback((targetPage) => {
    ...
    const onLeftMovePagesClick = useCallback(() => {
        ...
        if (firstVisiblePage - pageGroupSize < 1) {
            ...
            for (let i=firstVisiblePage - pageGroupSize; i<firstVisiblePage; i++) {
                newPageGroup.push(i);
            }
        }
    ...
    const onRightMovePagesClick = useCallback(() => {
        ...
        if (lastVisiblePage + pageGroupSize > maxPage) {
            ...
            for (let i=maxPage - pageGroupSize + 1; i<maxPage + 1; i++) {
                newPageGroup.push(i);
            }
        } else {
            for (let i=lastVisiblePage + 1; i<lastVisiblePage + pageGroupSize + 1; i++) {
                newPageGroup.push(i);
            }
        }
        setPageGroup(newPageGroup);
    ...
    return <nav aria-label="Node search result pages">
